...
export default function Terminologies() {
  ...
  async function getSearchedData() {
    try {
      ...
      const response = await Axios.get(
        `/search-terminology?key=${search}&page=${page}`
      );
  ...
  async function handleDelete(id) {
    try {
      await Axios.delete(`${baseUrl}/delete-terminology/${slideToDelete.id}`);
      setTerminologies((prev) => prev.filter((item) => item.id !== slideToDelete.id));
