...
export default function Links({ idf ,level}) {
  ...
  async function getSearchedData() {
    try {
      const response = await Axios.get(
        `/search-link?key=${search}&${idf}=${idValue}&page=${page}`
      );
  ...
  async function handleDelete(id) {
    try {
      await Axios.delete(`${baseUrl}/delete-link/${slideToDelete.id}`);
      setLinks((prev) => prev.filter((item) => item.id !== slideToDelete.id));
  ...
  const add_link = (idValue) => {
    navigate(`add_link`, { state: { idValue } });
  };

  const handleEdit = (link) => {
    navigate(`update_link/${link.id}`, { state: { link } });
  };
